...
namespace DotNetExport.dnlib.DotNet.Writer {
	...
	public interface IChunk {
		...
		/// <summary>
		/// Writes all data to <paramref name="dataWriter"/> at its current location. It's only
		/// called after <see cref="SetOffset"/> and <see cref="GetFileLength"/> have been called.
		/// You cannot assume that <paramref name="dataWriter"/>'s file position is the same as this
		/// chunk's file position.
		/// </summary>
		/// <param name="dataWriter">Destination</param>
		void WriteTo(DataWriter dataWriter);
	}
	...
	public static partial class Extensions {
		...
		internal static void WriteDataDirectory(this DataWriter writer, IChunk chunk) {
			if (chunk == null || chunk.GetVirtualSize() == 0)
				...
				if (chunk is ImportDirectory import)
                    writer.WriteUInt32(import.GetTableSize());
				else
