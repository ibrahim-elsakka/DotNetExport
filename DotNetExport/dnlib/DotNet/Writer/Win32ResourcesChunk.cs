...
using DotNetExport.dnlib.W32Resources;
	...
	public sealed class Win32ResourcesChunk : IReuseChunk {
		...
		public void WriteTo(DataWriter dataWriter) {
			...
			foreach (var dataHeader in dataHeaderList) {
				...
				offset += WriteTo(dataWriter, dataHeader);
			}
			...
			foreach (var s in stringsList) {
				...
				dataWriter.WriteUInt16((ushort)(bytes.Length / 2));
			...
			foreach (var data in dataList) {
				...
				reader.CopyTo(dataWriter, dataBuffer);
			}
		}

		uint WriteTo(DataWriter dataWriter, ResourceDirectory dir) {
			dataWriter.WriteUInt32(dir.Characteristics);
			dataWriter.WriteUInt32(dir.TimeDateStamp);
			dataWriter.WriteUInt16(dir.MajorVersion);
			dataWriter.WriteUInt16(dir.MinorVersion);
			...
			dataWriter.WriteUInt16((ushort)named.Count);
			dataWriter.WriteUInt16((ushort)ids.Count);
			...
			foreach (var d in named) {
				dataWriter.WriteUInt32(0x80000000 | stringsDict[d.Name.Name]);
				dataWriter.WriteUInt32(GetDirectoryEntryOffset(d));
			}

			foreach (var d in ids) {
				dataWriter.WriteInt32(d.Name.Id);
				dataWriter.WriteUInt32(GetDirectoryEntryOffset(d));
			}
		...
		uint WriteTo(DataWriter dataWriter, ResourceData dataHeader) {
			dataWriter.WriteUInt32((uint)rva + dataDict[dataHeader]);
			dataWriter.WriteUInt32((uint)dataHeader.CreateReader().Length);
			dataWriter.WriteUInt32(dataHeader.CodePage);
			dataWriter.WriteUInt32(dataHeader.Reserved);
			return 16;
		}
	}
}
