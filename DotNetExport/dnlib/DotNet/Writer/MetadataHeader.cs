...
namespace DotNetExport.dnlib.DotNet.Writer {
	...
	public sealed class MetadataHeader : IChunk {
		...
		public void WriteTo(DataWriter dataWriter) {
			dataWriter.WriteUInt32(options.Signature ?? MetadataHeaderOptions.DEFAULT_SIGNATURE);
			dataWriter.WriteUInt16(options.MajorVersion ?? 1);
			dataWriter.WriteUInt16(options.MinorVersion ?? 1);
			dataWriter.WriteUInt32(options.Reserved1 ?? 0);
			var s = GetVersionString();
			dataWriter.WriteInt32(Utils.AlignUp(s.Length, 4));
			dataWriter.WriteBytes(s);
			dataWriter.WriteZeroes(Utils.AlignUp(s.Length, 4) - s.Length);
			dataWriter.WriteByte((byte)(options.StorageFlags ?? 0));
			dataWriter.WriteByte(options.Reserved2 ?? 0);
			var heaps = this.heaps;
			dataWriter.WriteUInt16((ushort)heaps.Count);
			int count = heaps.Count;
			for (int i = 0; i < count; i++) {
				var heap = heaps[i];
				dataWriter.WriteUInt32((uint)(heap.FileOffset - offset));
				dataWriter.WriteUInt32(heap.GetFileLength());
				dataWriter.WriteBytes(s = GetAsciizName(heap.Name));
				if (s.Length > 32)
