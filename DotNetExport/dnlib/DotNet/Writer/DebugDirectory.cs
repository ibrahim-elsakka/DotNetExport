...
using DotNetExport.dnlib.DotNet.Pdb;
...
namespace DotNetExport.dnlib.DotNet.Writer {
	...
	public sealed class DebugDirectory : IReuseChunk {
		...
		public void WriteTo(DataWriter dataWriter) {
			uint offset = 0;
			foreach (var entry in entries) {
				dataWriter.WriteUInt32(entry.DebugDirectory.Characteristics);
				dataWriter.WriteUInt32(entry.DebugDirectory.TimeDateStamp);
				dataWriter.WriteUInt16(entry.DebugDirectory.MajorVersion);
				dataWriter.WriteUInt16(entry.DebugDirectory.MinorVersion);
				dataWriter.WriteUInt32((uint)entry.DebugDirectory.Type);
				uint length = entry.Chunk.GetFileLength();
				dataWriter.WriteUInt32(length);
				dataWriter.WriteUInt32(length == 0 ? 0 : (uint)entry.Chunk.RVA);
				dataWriter.WriteUInt32(length == 0 ? 0 : (uint)entry.Chunk.FileOffset);
				offset += HEADER_SIZE;
			}

			foreach (var entry in entries) {
				WriteAlign(dataWriter, ref offset);
				entry.Chunk.VerifyWriteTo(dataWriter);
