...
using DotNetExport.dnlib.Utils;
...
namespace DotNetExport.dnlib.DotNet.Writer {
	sealed class ManagedExportsWriter {
		...
		public readonly SdataChunk sdataChunk;
		readonly ExportDir exportDir;
		readonly List<VTableInfo> vtables;
		internal readonly List<MethodInfo> allMethodInfos;
		...
		sealed class ExportDir : IChunk {
			...
			void IChunk.WriteTo(DataWriter dataWriter) => throw new NotSupportedException();
		}
		...
		sealed class VtableFixupsChunk : IChunk {
			...
			public uint GetVirtualSize() => GetFileLength();
			public void WriteTo(DataWriter dataWriter) => owner.WriteVtableFixups(dataWriter);
		}
		...
		sealed class StubsChunk : IChunk {
			...
			public uint GetFileLength() => length;
			public uint GetVirtualSize() => GetFileLength();
			public void WriteTo(DataWriter dataWriter) => owner.WriteStubs(dataWriter);
		}

		public sealed class SdataChunk : IChunk {
			readonly ManagedExportsWriter owner;
			FileOffset offset;
			RVA rva;
			internal uint length;
			...
			public uint GetFileLength() => length;
			public uint GetVirtualSize() => GetFileLength();
			public void WriteTo(DataWriter dataWriter) => owner.WriteSdata(dataWriter);
		}
