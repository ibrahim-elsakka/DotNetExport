...
namespace DotNetExport.dnlib.DotNet.Writer {
	...
	public sealed class PEHeaders : IChunk {
		...
		public void WriteTo(DataWriter dataWriter) {
			startOffset = dataWriter.Position;

			// DOS header
			dataWriter.WriteBytes(dosHeader);

			// PE magic
			dataWriter.WriteInt32(0x00004550);

			// Image file header
			dataWriter.WriteUInt16((ushort)GetMachine());
			dataWriter.WriteUInt16((ushort)SectionsCount);
			...
			dataWriter.WriteUInt32(options.TimeDateStamp ?? PEHeadersOptions.CreateNewTimeDateStamp());
			dataWriter.WriteUInt32(options.PointerToSymbolTable ?? 0);
			dataWriter.WriteUInt32(options.NumberOfSymbols ?? 0);
			dataWriter.WriteUInt16((ushort)(Use32BitOptionalHeader() ? 0xE0U : 0xF0));
			dataWriter.WriteUInt16((ushort)GetCharacteristics());
			...
			if (Use32BitOptionalHeader()) {
				dataWriter.WriteUInt16((ushort)0x010B);
				dataWriter.WriteByte(options.MajorLinkerVersion ?? PEHeadersOptions.DEFAULT_MAJOR_LINKER_VERSION);
				dataWriter.WriteByte(options.MinorLinkerVersion ?? PEHeadersOptions.DEFAULT_MINOR_LINKER_VERSION);
				dataWriter.WriteUInt32(sectionSizes.SizeOfCode);
				dataWriter.WriteUInt32(sectionSizes.SizeOfInitdData);
				dataWriter.WriteUInt32(sectionSizes.SizeOfUninitdData);
				dataWriter.WriteUInt32(ep);
				dataWriter.WriteUInt32(sectionSizes.BaseOfCode);
				dataWriter.WriteUInt32(sectionSizes.BaseOfData);
				dataWriter.WriteUInt32((uint)imageBase);
				dataWriter.WriteUInt32(sectionAlignment);
				dataWriter.WriteUInt32(fileAlignment);
				dataWriter.WriteUInt16(options.MajorOperatingSystemVersion ?? 4);
				dataWriter.WriteUInt16(options.MinorOperatingSystemVersion ?? 0);
				dataWriter.WriteUInt16(options.MajorImageVersion ?? 0);
				dataWriter.WriteUInt16(options.MinorImageVersion ?? 0);
				dataWriter.WriteUInt16(options.MajorSubsystemVersion ?? 4);
				dataWriter.WriteUInt16(options.MinorSubsystemVersion ?? 0);
				dataWriter.WriteUInt32(options.Win32VersionValue ?? 0);
				dataWriter.WriteUInt32(sectionSizes.SizeOfImage);
				dataWriter.WriteUInt32(sectionSizes.SizeOfHeaders);
				checkSumOffset = dataWriter.Position;
				dataWriter.WriteInt32(0);	// CheckSum
				dataWriter.WriteUInt16((ushort)(options.Subsystem ?? PEHeadersOptions.DEFAULT_SUBSYSTEM));
				dataWriter.WriteUInt16((ushort)(options.DllCharacteristics ?? PEHeadersOptions.DefaultDllCharacteristics));
				dataWriter.WriteUInt32((uint)(options.SizeOfStackReserve ?? 0x00100000));
				dataWriter.WriteUInt32((uint)(options.SizeOfStackCommit ?? 0x00001000));
				dataWriter.WriteUInt32((uint)(options.SizeOfHeapReserve ?? 0x00100000));
				dataWriter.WriteUInt32((uint)(options.SizeOfHeapCommit ?? 0x00001000));
				dataWriter.WriteUInt32(options.LoaderFlags ?? 0x00000000);
				dataWriter.WriteUInt32(options.NumberOfRvaAndSizes ?? 0x00000010);
			}
			else {
				dataWriter.WriteUInt16((ushort)0x020B);
				dataWriter.WriteByte(options.MajorLinkerVersion ?? PEHeadersOptions.DEFAULT_MAJOR_LINKER_VERSION);
				dataWriter.WriteByte(options.MinorLinkerVersion ?? PEHeadersOptions.DEFAULT_MINOR_LINKER_VERSION);
				dataWriter.WriteUInt32(sectionSizes.SizeOfCode);
				dataWriter.WriteUInt32(sectionSizes.SizeOfInitdData);
				...
				dataWriter.WriteUInt32(ep);
				dataWriter.WriteUInt32(sectionSizes.BaseOfCode);
				dataWriter.WriteUInt64(imageBase);
				dataWriter.WriteUInt32(sectionAlignment);
				dataWriter.WriteUInt32(fileAlignment);
				dataWriter.WriteUInt16(options.MajorOperatingSystemVersion ?? 4);
				dataWriter.WriteUInt16(options.MinorOperatingSystemVersion ?? 0);
				...
				dataWriter.WriteUInt16((ushort)(options.Subsystem ?? PEHeadersOptions.DEFAULT_SUBSYSTEM));
				dataWriter.WriteUInt16((ushort)(options.DllCharacteristics ?? PEHeadersOptions.DefaultDllCharacteristics));
				dataWriter.WriteUInt64(options.SizeOfStackReserve ?? 0x0000000000400000);
				dataWriter.WriteUInt64(options.SizeOfStackCommit ?? 0x0000000000004000);
				dataWriter.WriteUInt64(options.SizeOfHeapReserve ?? 0x0000000000100000);
				dataWriter.WriteUInt64(options.SizeOfHeapCommit ?? 0x0000000000002000);
				dataWriter.WriteUInt32(options.LoaderFlags ?? 0x00000000);
				dataWriter.WriteUInt32(options.NumberOfRvaAndSizes ?? 0x00000010);
			}

			dataWriter.WriteDataDirectory(ExportDirectory);
			dataWriter.WriteDataDirectory(ImportDirectory);
			dataWriter.WriteDataDirectory(Win32Resources);
			dataWriter.WriteDataDirectory(null);	// Exception table
			dataWriter.WriteDataDirectory(null);	// Certificate table
			dataWriter.WriteDataDirectory(RelocDirectory);
			dataWriter.WriteDebugDirectory(DebugDirectory);
			dataWriter.WriteDataDirectory(null);	// Architecture-specific data
			dataWriter.WriteDataDirectory(null);	// Global pointer register RVA
			dataWriter.WriteDataDirectory(null);	// Thread local storage
			dataWriter.WriteDataDirectory(null);	// Load configuration table
			dataWriter.WriteDataDirectory(null);	// Bound import table
			dataWriter.WriteDataDirectory(ImportDirectory.ImportAddress);
			dataWriter.WriteDataDirectory(null);	// Delay import descriptor
			dataWriter.WriteDataDirectory(ImageCor20Header);
			dataWriter.WriteDataDirectory(null);	// Reserved
			...
			foreach (var section in sections) {
				if (section.GetVirtualSize() != 0)
					rva += section.WriteHeaderTo(dataWriter, fileAlignment, sectionAlignment, rva);
				else
					emptySections++;
			}
			if (emptySections != 0)
				dataWriter.Position += emptySections * 0x28;
		}
